...
{
      public static TaskRunnerOptions CreateOptions(
        int count = 1,
        int maxParallel = 1,
        int maxCount = 1,
    ...
    private static TTaskRunner InstantiateTaskRunner<TTaskRunner, TAction>(
        string name, TAction action, TaskRunnerOptions options)
        where TTaskRunner : ITaskRunner
        where TAction : Delegate
    {
        return (TTaskRunner)Activator.CreateInstance(typeof(TTaskRunner), name, action, options)!
               ?? throw new InvalidOperationException($"Failed to create an instance of {typeof(TTaskRunner).Name}.");
    }

    public static TTaskRunner Create<TTaskRunner, TAction>(
        string name, TAction action, TaskRunnerOptions? options = null,
        int count = 1, int maxParallel = 1, bool endless = false, int delay = 1000)
        where TTaskRunner : ITaskRunner
        where TAction : Delegate
    {
    ...
    public static TTaskRunner CreateAndStart<TTaskRunner, TAction>(
        string name, TAction action, TaskRunnerOptions? options = null,
        int count = 1, int maxParallel = 1, bool endless = false, int delay = 1000)
        where TTaskRunner : ITaskRunner
        where TAction : Delegate
    ...
    public static TTaskRunner CreateAndFire<TTaskRunner, TAction>(
        string name,
        TAction action,
        RequestTaskFire fire,
